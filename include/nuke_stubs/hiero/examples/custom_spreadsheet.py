...
class CustomSpreadsheetColumns(QtCore.QObject):
  ...
  arriColourSpaces = ['Video - Rec709','LogC - Camera Native','Video - P3-DCI','ACES','LogC - Film','LogC - Wide Gamut']
  r3dColourSpaces = ['Half Float Linear','Rec709','REDspace','REDlog','PDlog685','PDlog985','CustomPDlog','REDgamma','SRGB','REDlogFilm','REDgamma2','REDgamma3', 'REDgamma4']
  ...
  def setStatusFromMenuSelection(self, menuSelectionStatus):
    ...
    if len(selectedTracks)>=1:
      for track in selectedTracks:
        selectedShots+=[item for item in list(track.items()) if (isinstance(item,hiero.core.TrackItem))]
