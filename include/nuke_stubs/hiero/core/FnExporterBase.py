  ...
  def inputRange(self, ignoreHandles=False, ignoreRetimes=False, clampToSource=True):
    ...
    if isinstance(self._item, (TrackItem, Clip)):
      if self._cutHandles is None:
        ...
        isFreezeFrame = (ignoreRetimes == False and self._retime and ti.playbackSpeed() == 0.0)
        ...
        log.debug("  ignoreRetimes = %s, self._retime = %s" % (str(ignoreRetimes), str(self._retime)))
